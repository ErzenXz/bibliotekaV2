...
function addBook() {
    ...
    let bvideo = document.getElementById("bookvideo").value;
    ...
    if (bname == "") {
        ...
        firebase.database().ref('movies/' + keyA).set({
        ...
        document.getElementById("bookvideo").value = "";
        swal.fire("Movie has been succesfuly added.");
    }
...
firebase.database().ref("books/").on("child_removed", function (snapshot) {
    swal.fire("Movie has been succesfully deleted.")
});
...
db.collection("movies").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
        ...
        let bmovie = doc.data().bookMovieURL;
        let ms = doc.data().ms;
        let time = doc.data().time;
        let key = doc.id;
        bdecs = bdecs.replace(/["']/g, "");

        addBookToUL(name, email, bname, bauthor, bdecs, bimage, ms, time, key, video);
    });
});


function addBookToUL(name, email, bname, bauthor, bdecs, bimage, btime, bt, key, video) {
    ...
    li.innerHTML = `<b>${bname}</b>      - by ${name} (${email}) - ${time_ago(btime)} <button onclick="openMenu('${name}','${email}','${bname}', '${bauthor}', '${bdecs}', '${bimage}', '${btime}', '${bt}', '${key}', '${video}')">Details</button>`;

    ul.appendChild(li);
}

function openMenu(name, email, bname, bauthor, bdecs, bimage, btime, bt, key, video) {
    ...
    document.getElementById("bookVideo").innerHTML = `<videowidth="320px height="auto">
    ...
    document.getElementById('but1').setAttribute("onclick", `approveBook('${name}', '${email}', '${bname}', '${bdecs}', '${bauthor}', '${bimage}', '${key}', '${video}')`);
...
function approveBook(name, email, bname, bdesc, bauthor, bimage, key, video) {
    ...
    db.collection("books").doc(key).delete().then(() => {
        console.log("Movie successfully approved!");
        document.getElementById(key).remove();
        closeMenu();
    }).catch((error) => {
...
function denyBook(name, email, book) {
    db.collection("movies").doc(book).delete().then(() => {
